"""
Bloom Filter implementation for TinySift.
...
The Bloom Filter provides the following guarantees:
1. Space Complexity: O(m) where m is the number of bits in the filter
2. Add Time: O(k) where k is the number of hash functions
3. Query Time: O(k) where k is the number of hash functions
4. No false negatives: If an item is in the set, the filter will always return True
5. Configurable false positive rate: The rate is determined by the filter size and hash count
...
class BloomFilter(StreamSummary[T, bool]):
    ...
    The false positive rate is configurable through the size of the bit array and
    the number of hash functions. The optimal number of hash functions (k) is
    calculated as (m/n)*ln(2) where m is the bit array size and n is the expected
    number of elements.
    ...
    def estimate_size(self) -> int:
        ...
        """
        # Base size of the object itself (attributes, dict, etc.)
        ...
        # Add size of buffer explicitly if getsizeof doesn't capture it fully?
        # size += self._bytes.buffer_info()[1] * self._bytes.itemsize # Usually covered
